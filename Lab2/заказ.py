...
class Заказ:
    ...
    def __str__(self):
        ...
        for пицца in self.заказанные_пиццы:
            message += (f"{self.заказанные_пиццы.index(пицца)+1}. {пицца}\n")

        return (f"Заказ №{Заказ.счетчик_заказов}\n{message}")
    ...
    def выполнить(self):
        ...
        """
        эвент_пиццы = self.заказанные_пиццы[0]

        for пицца in self.заказанные_пиццы:
            print(f"Заказ поступил на выполнение...\n{'/'*80}\n"
                  f"{self.заказанные_пиццы.index(пицца)+1}. {пицца.название}")
            эвент_пиццы.подготовить()
            time.sleep(1)
            Пицца.испечь()
            time.sleep(1)
            Пицца.нарезать()
