class Roman:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Roman):
            return (self.to_roman(self.arabic + other.arabic))
        else:
    ...
    def __check_arabic(value):
        ...
        if value < Roman.ARABIC_MIN or value > Roman.ARABIC_MAX:
            raise ValueError("Полученное значение не принадлежит [ARABIC_MIN]&[ARABIC_MAX]")
    ...
    def __check_roman(value):
        ...
        counter = 0
        for char in value:
            for element in Roman.LETTERS:
                if char == element:
                    counter += 1
                else: pass            
            if counter == 0: 
                raise ValueError("Недопустимый символ в списке LETTERS")
            counter = 0
